    ...
    def latest_extended_sync(self) -> dict[str, Any] | None:
        ...
        with self._db() as conn:
            latest_run = conn.execute(
                """
                SELECT status, plan_count, detail_count, product_row_count, material_row_count,
                       original_video_row_count, error_count, snapshot_time
            ...
            if latest_run is None:
                candidate = self._latest_extended_sync_run(conn)
                if candidate and str(candidate["snapshot_time"] or "").strip()[:10] == snapshot_time[:10]:
        ...
        run_payload = dict(latest_run) if latest_run else {}
        return {
            "snapshot_time": snapshot_time,
            "status": str(run_payload.get("status") or "ok").strip() or "ok",
            "plan_count": int(run_payload.get("plan_count", len(plan_ids)) or len(plan_ids)),
            "detail_count": int(run_payload.get("detail_count", material_row_count) or material_row_count),
            "product_row_count": int(run_payload.get("product_row_count", len(products)) or len(products)),
            "material_row_count": int(run_payload.get("material_row_count", material_row_count) or material_row_count),
            "original_video_row_count": int(run_payload.get("original_video_row_count", original_video_count) or original_video_count),
            "error_count": int(run_payload.get("error_count", 0) or 0),
        }
