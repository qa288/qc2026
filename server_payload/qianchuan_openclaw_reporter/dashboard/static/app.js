...
const PASSIVE_ACTIVE_VIEW_REFRESH_SKIP = new Set(["materials", "team-materials", "comments"]);
const MATERIAL_VIEW_MODE_KEY = "material-view-mode";
const MATERIAL_VIEW_PERFORMANCE = "performance";
const MATERIAL_VIEW_LIBRARY = "library";
const MATERIAL_VIEW_RELATION = "relation";
const MATERIAL_TOTAL_PAY_TYPES = new Set(["VIDEO", "IMAGE", "TITLE"]);
const MATERIAL_SETTLED_TYPES = new Set(["VIDEO", "IMAGE"]);

function clampPositiveInt(value, fallback, min = 1, max = 24 * 60) {
  const normalizedFallback = Number.isFinite(Number(fallback)) ? Number(fallback) : min;
  const parsed = Number.parseInt(value, 10);
  if (!Number.isFinite(parsed) || parsed <= 0) {
    return Math.max(min, Math.min(normalizedFallback, max));
  }
  return Math.max(min, Math.min(parsed, max));
}

function materialHotSyncMinutes(config = state.oceanEngineConfig) {
  return clampPositiveInt(config?.material_sync_interval_minutes, 10, 1, 120);
}

function materialWarmSyncMinutes(config = state.oceanEngineConfig) {
  const hotMinutes = materialHotSyncMinutes(config);
  const fallback = Math.max(hotMinutes, 30);
  return clampPositiveInt(config?.material_sync_warm_interval_minutes, fallback, hotMinutes, 24 * 60);
}

function dashboardRefreshHintText(config = state.oceanEngineConfig) {
  return `采集 1 分钟 · 素材热刷新 ${materialHotSyncMinutes(config)} 分钟 · 温补 ${materialWarmSyncMinutes(config)} 分钟 · 页面缓存 60 秒`;
}
...
const state = {
  ...
  materialRelationDetailCache: {},
  materialRelationDetailRequestToken: 0,
  selectedUserId: null,
  userEditorMode: "edit",
...
const historySyncCard = document.getElementById("historySyncCard");
...
const materialModeSwitch = document.getElementById("materialModeSwitch");
...
const materialSemanticsNotice = document.getElementById("materialSemanticsNotice");
...
function requestDisplayScope() {
  const preferredScope = normalizeDisplayScope(state.displayScope);
  if (!state.session) {
...
function splitDateTimeParts(value) {
  ...
  const [datePart = "", timePart = ""] = String(normalized).split(" ");
  return {
    datePart: datePart || normalized,
    timePart: timePart || "",
  };
}

function renderDateTimeCell(value, fallback = "未记录") {
  const parts = splitDateTimeParts(value);
  if (!parts) {
    return `
      <div class="cell-primary">${escapeHtml(fallback)}</div>
  ...
  return `
    <div class="cell-primary">${escapeHtml(parts.datePart)}</div>
    <div class="cell-subline mono">
      <span class="cell-subitem">${escapeHtml(parts.timePart || "--:--:--")}</span>
    </div>
  `;
}
...
function materialRowIdentity(row) {
  return String(row?.material_row_key || row?.material_key || "").trim();
}
...
function enrichAccountRow(row, accountMetrics) {
  ...
  const totalPayAmount = Number(metrics.total_pay_amount ?? row?.total_pay_amount ?? 0);
  const settledPayAmount = Number(metrics.settled_pay_amount ?? row?.settled_pay_amount ?? 0);
  const orderCount = Number(row?.order_count ?? metrics.order_count ?? 0);
  const settledOrderCount = Number(metrics.settled_order_count ?? row?.settled_order_count ?? 0);
  const refundAmount1h = Number(metrics.refund_amount_1h ?? row?.refund_amount_1h ?? 0);
  return {
    ...row,
    stat_cost: statCost,
    ...
    settled_order_count: settledOrderCount,
    refund_amount_1h: refundAmount1h,
    plan_count: Number(metrics.plan_count ?? row?.plan_count ?? 0),
    top_plan_name: String(metrics.top_plan_name ?? row?.top_plan_name ?? "").trim(),
...
async function fetchMaterialRelationDetail(row, force = false) {
  ...
  const response = await fetch(`/api/material-relation-detail?${params.toString()}`, {
    method: "POST",
...
function materialPlanRelationItemsNeedResolve(items) {
  return (items || []).some((item) => !String(item?.ad_name || "").trim() || !String(item?.advertiser_name || "").trim());
}

function materialAccountRelationItemsNeedResolve(items) {
  return (items || []).some((item) => !String(item?.advertiser_name || "").trim() || (!String(item?.top_plan_name || "").trim() && Number(item?.plan_count || 0) > 0));
}
...
function normalizeMaterialViewMode(value) {
  const normalized = String(value || "").trim().toLowerCase();
  if (normalized === MATERIAL_VIEW_LIBRARY) return MATERIAL_VIEW_LIBRARY;
  return MATERIAL_VIEW_PERFORMANCE;
}

function materialViewMode() {
...
function materialFilter() {
  return {
    ...
    materialMode: materialViewMode(),
    variant: `material:${materialViewMode()}`,
  };
}
...
function materialPayloadKey(filter, overrides = {}) {
  const requestState = materialRequestState(overrides);
  return `${performanceFilterKey(filter)}:page:${requestState.page}:size:${requestState.pageSize}:sort:${requestState.sortKey}:${requestState.sortDir}:search:${requestState.search}`;
}

function teamMaterialPayloadKey(filter, overrides = {}) {
  const requestState = teamMaterialRequestState(overrides);
  return `${performanceFilterKey(filter)}:team-page:${requestState.page}:size:${requestState.pageSize}:sort:${requestState.sortKey}:${requestState.sortDir}:search:${requestState.search}`;
}

function activeMaterialPreviewMode() {
  return String(state.activeView || "").trim() === "materials"
    ? materialViewMode()
    : MATERIAL_VIEW_PERFORMANCE;
}

function activeMaterialPreviewFilter() {
  return String(state.activeView || "").trim() === "materials"
    ? materialFilter()
    : sectionFilter("teamMaterial");
}

function performanceFilterKey(filter) {
  const normalized = normalizeRangeFilter(filter);
  const section = String(filter?.section || "all").trim() || "all";
  const variant = String(filter?.variant || "").trim();
  const prefix = requestDisplayScope();
  if (normalized.mode === "custom") {
    return `${prefix}:${section}${variant ? `:${variant}` : ""}:custom:${normalized.start}:${normalized.end}`;
  }
  return `${prefix}:${section}${variant ? `:${variant}` : ""}:${normalized.mode}`;
}

function sectionFilter(sectionKey) {
  return {
    ...normalizeRangeFilter(state.performanceFilters[sectionKey] || { mode: "day" }),
    section: String(sectionKey || "").trim() || "all",
  };
}
...
function shouldSkipActiveViewRefresh(view = state.activeView) {
  return PASSIVE_ACTIVE_VIEW_REFRESH_SKIP.has(String(view || "").trim());
}
...
function syncMaterialViewModeControls() {
  const mode = materialViewMode();
  const operator = isOperator();
  materialModeSwitch?.querySelectorAll(".range-button").forEach((button) => {
    button.classList.toggle("active", normalizeMaterialViewMode(button.dataset.mode) === mode);
  });
  if (materialsPanelTitle) {
    if (mode === MATERIAL_VIEW_LIBRARY) {
      materialsPanelTitle.textContent = operator ? "我的素材资产库" : "素材资产库";
    } else if (mode === MATERIAL_VIEW_RELATION) {
      materialsPanelTitle.textContent = operator ? "我的素材关系核对" : "素材关系核对";
    } else {
      materialsPanelTitle.textContent = operator ? "我的素材表现" : "素材表现";
    }
  }
  if (materialSearch) {
    materialSearch.placeholder = operator
      ? (
        mode === MATERIAL_VIEW_LIBRARY
          ? "搜索素材名称 / 素材ID / 视频ID"
          : (mode === MATERIAL_VIEW_RELATION ? "搜索素材 / 素材ID / 账户 / 计划" : "搜索素材名称")
      )
      : (
        mode === MATERIAL_VIEW_LIBRARY
          ? "搜索素材 / 素材ID / 归属账户 / 计划"
          : (mode === MATERIAL_VIEW_RELATION ? "搜索素材 / 素材ID / 账户 / 计划 / 达人" : "搜索素材")
      );
  }
}
...
function formatDateWindowMeta(payload) {
  ...
  const backfillQueued =
    payload.history_backfill_pending
    && Number(payload.missing_history_days || 0) > 0
    && Boolean(payload.history_backfill_queued);
  const correctionQueued =
    payload.history_correction_pending
    && Number(payload.missing_correction_days || 0) > 0
    && Boolean(payload.history_correction_queued);
  const label = payload.range_label || RANGE_LABELS[payload.range_key] || "当前";
  const maintenanceHints = [];
  const recentMissingDays = Number(payload.history_recent_missing_days || 0);
  const stableRecentHistory = recentMissingDays === 0 && payload.yesterday_finalized !== false;
  if (payload.history_backfill_pending && Number(payload.missing_history_days || 0) > 0) {
    const missingDays = Number(payload.missing_history_days || 0);
    const monthLikeWindow = ["month", "custom"].includes(String(payload.range_key || "").trim().toLowerCase());
    maintenanceHints.push(
      monthLikeWindow && stableRecentHistory
        ? `更早历史补齐中（缺 ${missingDays} 天，昨日和近7天已稳定；当前${label}总量仍会变化${backfillQueued ? "，已排队" : ""}）`
        : `历史补齐中（缺 ${missingDays} 天${backfillQueued ? "，已排队" : ""}）`,
    );
  }
  if (payload.history_correction_pending && Number(payload.missing_correction_days || 0) > 0) {
    const correctionParts = [`待校正 ${Number(payload.missing_correction_days || 0)} 天`];
    if (payload.yesterday_finalized === false) {
      correctionParts.push("昨日日结待完成");
    }
    if (correctionQueued) {
      correctionParts.push("已排队");
    }
    maintenanceHints.push(`历史校正中（${correctionParts.join("，")}）`);
  }
  const maintenanceHint =
    maintenanceHints.length > 0
      ? ` · ${maintenanceHints.join("；")}`
      : "";
  if (payload.query_start_date && payload.query_end_date) {
    if (payload.query_start_date === payload.query_end_date) {
      return `统计范围：${label} · ${payload.query_start_date}${maintenanceHint}`;
    }
    return `统计范围：${label} · ${payload.query_start_date} 至 ${payload.query_end_date}${maintenanceHint}`;
  }
  return `统计范围：${label} · ${payload.window_start} - ${payload.window_end}${maintenanceHint}`;
}

function formatMaterialTodaySystemText(payload) {
  const running = Boolean(payload?.today_hot_running);
  const missing = Boolean(payload?.today_hot_missing);
  const stale = Boolean(payload?.today_hot_stale);
  const lastSnapshot = String(payload?.today_hot_last_snapshot_time || "").trim();
  const lagSeconds = Number(payload?.today_hot_lag_seconds || 0);
  const lagMinutes = lagSeconds > 0 ? Math.max(1, Math.ceil(lagSeconds / 60)) : 0;
  if (running) {
    return lastSnapshot
      ? `today 热同步中（当前展示 ${lastSnapshot} 快照）`
      : "today 热同步中（正在生成今日素材）";
  }
  if (missing) {
    return "today 素材待生成";
  ...
  if (stale) {
    return lagMinutes > 0
      ? `today 素材待刷新（已落后约 ${formatNumber(lagMinutes)} 分钟）`
      : "today 素材待刷新";
  }
  return "today 热层正常";
}

function formatMaterialTodayStatusSuffix(payload) {
  const running = Boolean(payload?.today_hot_running);
  const missing = Boolean(payload?.today_hot_missing);
  const stale = Boolean(payload?.today_hot_stale);
  if (!running && !missing && !stale) {
    return "";
  }
  return ` · ${formatMaterialTodaySystemText(payload)}`;
}

function formatMaterialHistorySystemText(payload) {
  const backfillPending = payload?.history_backfill_pending && Number(payload?.missing_history_days || 0) > 0;
  const correctionPending = payload?.history_correction_pending && Number(payload?.missing_correction_days || 0) > 0;
  const label = payload?.range_label || RANGE_LABELS[payload?.range_key] || "当前窗口";
  const recentMissingDays = Number(payload?.history_recent_missing_days || 0);
  const stableRecentHistory = recentMissingDays === 0 && payload?.yesterday_finalized !== false;
  if (backfillPending) {
    const queued = payload?.history_backfill_queued ? "，已排队" : "";
    const monthLikeWindow = ["month", "custom"].includes(String(payload?.range_key || "").trim().toLowerCase());
    if (monthLikeWindow && stableRecentHistory) {
      return `更早历史补齐中（缺 ${formatNumber(payload?.missing_history_days || 0)} 天，昨日和近7天已稳定；当前${label}总量仍会变化${queued}）`;
    }
    return `历史补齐中（缺 ${formatNumber(payload?.missing_history_days || 0)} 天${queued}）`;
  }
  if (correctionPending) {
    const parts = [`待校正 ${formatNumber(payload?.missing_correction_days || 0)} 天`];
    if (payload?.yesterday_finalized === false) {
      parts.push("昨日日结待完成");
    }
    if (payload?.history_correction_queued) {
      parts.push("已排队");
    }
    if (["month", "custom"].includes(String(payload?.range_key || "").trim().toLowerCase()) && stableRecentHistory) {
      parts.push(`当前${label}总量仍可能小幅变化`);
    }
    return `历史校正中（${parts.join("，")}）`;
  }
  return payload?.yesterday_finalized === false ? "昨日日结待完成" : "历史已补齐";
}

function materialHistoryOverviewState(payload) {
  const backfillPending = Boolean(payload?.history_backfill_pending) && Number(payload?.missing_history_days || 0) > 0;
  const correctionPending = Boolean(payload?.history_correction_pending) && Number(payload?.missing_correction_days || 0) > 0;
  const recentMissingDays = Number(payload?.history_recent_missing_days || 0);
  const missingHistoryDays = Number(payload?.missing_history_days || 0);
  const missingCorrectionDays = Number(payload?.missing_correction_days || 0);
  const yesterdayFinalized = payload?.yesterday_finalized !== false;
  const stableRecentHistory = recentMissingDays === 0 && yesterdayFinalized;
  if (!backfillPending && !correctionPending) {
    return {
      tone: "",
      label: "稳定",
      value: "近30天完整",
      copy: "素材表现与关系历史都已补齐，当前窗口不再因历史补数继续变动。",
    };
  }
  if (backfillPending) {
    if (stableRecentHistory) {
      return {
        tone: "warn",
        label: "补齐中",
        value: `更早缺 ${formatNumber(missingHistoryDays)} 天`,
        copy: "昨日和近7天已稳定；当前30天窗口总量仍会随着更早历史补齐继续变化。",
      };
    }
    return {
      tone: "danger",
      label: "待补齐",
      value: `缺 ${formatNumber(missingHistoryDays)} 天`,
      copy: "近7天内仍有历史缺口，当前窗口还不能视为最终值。",
    };
  }
  return {
    tone: stableRecentHistory ? "warn" : "danger",
    label: stableRecentHistory ? "校正中" : "待校正",
    value: `待校正 ${formatNumber(missingCorrectionDays)} 天`,
    copy: stableRecentHistory
      ? "近7天已稳定，但历史校正尚未完成，当前30天窗口仍可能小幅变化。"
      : "近7天内仍有未完成校正，当前窗口仍可能继续变化。",
  };
}

function materialNoticeState(payload) {
  const mode = normalizeMaterialViewMode(payload?.material_mode || materialViewMode());
  const historyPending = Boolean(payload?.history_backfill_pending) && Number(payload?.missing_history_days || 0) > 0;
  const correctionPending = Boolean(payload?.history_correction_pending) && Number(payload?.missing_correction_days || 0) > 0;
  if (mode === MATERIAL_VIEW_RELATION && (historyPending || correctionPending)) {
    const recentMissingDays = Number(payload?.history_recent_missing_days || 0);
    const stableRecentHistory = recentMissingDays === 0 && payload?.yesterday_finalized !== false;
    return {
      text: formatMaterialHistorySystemText(payload),
      tone: stableRecentHistory ? "warn" : "danger",
    };
  }
  const noticeText = String(payload?.metrics_semantics?.notice || "").trim();
  if (!noticeText) {
    return { text: "", tone: "neutral" };
  }
  return {
    text: noticeText,
    tone: payload?.metrics_semantics?.reconcilable_to_account_summary ? "neutral" : "warn",
  };
}
...
function materialRangePayload(filter) {
  return state.materialPayloads[materialPayloadKey(filter)] || null;
}

function teamMaterialRangePayload(filter) {
  return state.teamMaterialPayloads[teamMaterialPayloadKey(filter)] || null;
}
...
function materialCacheSnapshotToken(filter) {
  const normalized = normalizeRangeFilter(filter);
  return normalized.mode === "day" ? latestMaterialSnapshotToken() : "";
}

function materialCacheTtlMs(filter) {
  const normalized = normalizeRangeFilter(filter);
  return normalized.mode === "day" ? MATERIAL_CACHE_TTL_MS : 5 * 60 * 1000;
}

function materialRowsForCurrentFilter() {
  return (materialRangePayload(materialFilter())?.items || []).map((row) => enrichMaterialRow(row));
}
...
function rangeLabel(filter) {
  const normalized = {
    ...normalizeRangeFilter(filter),
    section: String(filter?.section || "").trim(),
  };
  if (normalized.mode === "custom") {
    return "指定日期范围";
  }
...
function renderSystemCards(latest, extendedSync, tokenInfo) {
  ...
  const materialTodayStatus = extendedSync?.materialTodayStatus || latest?.materialTodayStatus || {};
  const materialTodayText = formatMaterialTodaySystemText(materialTodayStatus);
  const detailStatus = extendedSync?.status || "未同步";
  const detailTone = materialTodayStatus?.today_hot_running || materialTodayStatus?.today_hot_pending
    ? "warn"
    : detailStatus === "ok"
      ? "ok"
      : detailStatus === "partial"
        ? "warn"
        : "danger";
  const detailLabel = materialTodayStatus?.today_hot_running
    ? "同步中"
    : materialTodayStatus?.today_hot_pending
      ? "待刷新"
      : detailStatus === "ok"
        ? "完整"
        : detailStatus === "partial"
  ...
  const materialHistoryStatus = latest?.materialHistoryStatus || {};
  const materialHistoryText = formatMaterialHistorySystemText(materialHistoryStatus);
  const materialHistoryOverview = materialHistoryOverviewState(materialHistoryStatus);
  const hotSyncMinutes = materialHotSyncMinutes();
  const warmSyncMinutes = materialWarmSyncMinutes();
  ...
  detailSyncCard.innerHTML = `
    <div class="system-card-head">
      <span class="system-card-label">素材热同步</span>
      <span class="system-pill ${detailTone === "warn" ? "warn" : detailTone === "danger" ? "danger" : ""}">${escapeHtml(detailLabel)}</span>
    </div>
    <div class="system-card-value">${escapeHtml(extendedSync?.snapshot_time || "等待素材热同步")}</div>
    <div class="system-card-copy">today 素材表现按 ${hotSyncMinutes} 分钟热刷新；近 7 天 / 30 天按 ${warmSyncMinutes} 分钟温补；${escapeHtml(materialTodayText)}；${escapeHtml(materialHistoryText)}。</div>
    <div class="system-stat-grid">
      ...
      <div class="system-stat"><span>today 状态</span><strong>${escapeHtml(materialTodayText)}</strong></div>
      <div class="system-stat"><span>历史状态</span><strong>${escapeHtml(materialHistoryText)}</strong></div>
    </div>
  `;

  if (historySyncCard) {
    historySyncCard.innerHTML = `
      <div class="system-card-head">
        <span class="system-card-label">素材历史</span>
        <span class="system-pill ${materialHistoryOverview.tone === "warn" ? "warn" : materialHistoryOverview.tone === "danger" ? "danger" : ""}">${escapeHtml(materialHistoryOverview.label)}</span>
      </div>
      <div class="system-card-value">${escapeHtml(materialHistoryOverview.value)}</div>
      <div class="system-card-copy">${escapeHtml(materialHistoryOverview.copy)}</div>
      <div class="system-stat-grid">
        <div class="system-stat"><span>缺历史</span><strong class="mono">${formatNumber(materialHistoryStatus?.missing_history_days || 0)}</strong></div>
        <div class="system-stat"><span>近7天缺口</span><strong class="mono">${formatNumber(materialHistoryStatus?.history_recent_missing_days || 0)}</strong></div>
        <div class="system-stat"><span>待校正</span><strong class="mono">${formatNumber(materialHistoryStatus?.missing_correction_days || 0)}</strong></div>
        <div class="system-stat"><span>昨日日结</span><strong>${materialHistoryStatus?.yesterday_finalized === false ? "待完成" : "已稳定"}</strong></div>
      </div>
    `;
  }
}
...
function sortMaterialRows(rows, sortState) {
  if (String(sortState?.key || "") !== "create_time") {
    return sortRows(rows, sortState);
  }
  return [...rows].sort((left, right) => {
    const leftTime = String(left?.create_time || "").trim();
    const rightTime = String(right?.create_time || "").trim();
    if (!leftTime && !rightTime) {
      return compareValues(left?.material_name, right?.material_name, "asc");
    }
    if (!leftTime) return 1;
    if (!rightTime) return -1;
    return compareValues(leftTime, rightTime, sortState.dir);
  });
}
...
function buildAccountTableSummary(rows) {
  const summary = {
  ...
  rows.forEach((row) => {
    summary.stat_cost += Number(row?.stat_cost || 0);
    summary.pay_amount += Number(row?.pay_amount || 0);
    summary.total_pay_amount += Number(row?.total_pay_amount || 0);
    summary.settled_pay_amount += Number(row?.settled_pay_amount || 0);
    summary.order_count += Number(row?.order_count || 0);
    summary.settled_order_count += Number(row?.settled_order_count || 0);
    summary.refund_amount_1h += Number(row?.refund_amount_1h || 0);
    summary.plan_count += Number(row?.plan_count || 0);
  });
  summary.stat_cost = Number(summary.stat_cost.toFixed(2));
  summary.pay_amount = Number(summary.pay_amount.toFixed(2));
  summary.total_pay_amount = Number(summary.total_pay_amount.toFixed(2));
  summary.settled_pay_amount = Number(summary.settled_pay_amount.toFixed(2));
  summary.refund_amount_1h = Number(summary.refund_amount_1h.toFixed(2));
  summary.roi = summary.stat_cost > 0 ? Number((summary.pay_amount / summary.stat_cost).toFixed(2)) : 0;
  summary.settled_roi = summary.stat_cost > 0 ? Number((summary.settled_pay_amount / summary.stat_cost).toFixed(2)) : 0;
  summary.pay_order_cost = summary.order_count > 0 ? Number((summary.stat_cost / summary.order_count).toFixed(2)) : null;
  summary.settled_amount_rate = summary.total_pay_amount > 0
    ? Number((summary.settled_pay_amount / summary.total_pay_amount * 100).toFixed(2))
    : null;
  summary.refund_rate_1h = summary.total_pay_amount > 0
    ? Number((summary.refund_amount_1h / summary.total_pay_amount * 100).toFixed(2))
    : null;
  return summary;
}

function buildPlanTableSummary(rows) {
  const summary = {
...
function formatAccountHeaderValue(columnKey, summary) {
  switch (columnKey) {
    case "stat_cost":
    case "pay_amount":
    case "total_pay_amount":
    case "settled_pay_amount":
      return formatMoney(summary[columnKey]);
    case "roi":
    case "settled_roi":
      return formatRate(summary[columnKey]);
    case "order_count":
    case "settled_order_count":
    case "plan_count":
      return formatNumber(summary[columnKey]);
    case "pay_order_cost":
      return summary.pay_order_cost === null ? "-" : formatMoney(summary.pay_order_cost);
    case "settled_amount_rate":
      return summary.settled_amount_rate === null ? "-" : formatPercent(summary.settled_amount_rate);
    case "refund_rate_1h":
      return summary.refund_rate_1h === null ? "-" : formatPercent(summary.refund_rate_1h);
    default:
      return "";
  }
}

function accountStatusClass(statusText) {
  if (statusText === "查询失败") return "status-chip status-chip-error";
  if (statusText === "计划聚合") return "status-chip status-chip-fallback";
  return "status-chip status-chip-ok";
}

function makeAccountHeader(columns, sortState, summaryText, summaryMetrics) {
  return `
    <thead>
      <tr>
        ${columns.map((column) => {
          const active = sortState.key === column.key;
          const hint = !column.sortable ? "" : active ? (sortState.dir === "asc" ? "↑" : "↓") : "↕";
          const value = column.key === "advertiser_name" ? summaryText : formatAccountHeaderValue(column.key, summaryMetrics);
          return `
            <th data-key="${column.key}" class="account-table-th ${column.sortable ? "is-sortable" : ""}">
              <div class="account-table-head ${column.key === "advertiser_name" ? "is-primary" : ""}">
                <div class="account-table-head-main">
                  <span class="account-table-head-label">${column.label}</span>
                  ${column.sortable ? `<span class="account-table-sort-indicator" aria-hidden="true">${hint}</span>` : ""}
                </div>
                ${value
                  ? `<div class="account-table-head-total mono ${column.key === "advertiser_name" ? "is-account-total" : "is-metric-total"}">${value}</div>`
                  : `<div class="account-table-head-total is-empty" aria-hidden="true"></div>`}
              </div>
            </th>
          `;
        }).join("")}
...
function formatPlanHeaderValue(columnKey, summary) {
  switch (columnKey) {
    case "stat_cost":
    case "pay_amount":
    case "total_pay_amount":
...
function makePlanHeader(columns, sortState, summaryText, summaryMetrics) {
  return `
    <thead>
      <tr>
        ${columns.map((column) => {
          const active = sortState.key === column.key;
          const hint = !column.sortable ? "" : active ? (sortState.dir === "asc" ? "↑" : "↓") : "↕";
          const value = column.key === "ad_name" ? summaryText : formatPlanHeaderValue(column.key, summaryMetrics);
          return `
            <th data-key="${column.key}" class="account-table-th ${column.sortable ? "is-sortable" : ""}">
              <div class="account-table-head ${column.key === "ad_name" ? "is-primary" : ""}">
                <div class="account-table-head-main">
                  <span class="account-table-head-label">${column.label}</span>
                  ${column.sortable ? `<span class="account-table-sort-indicator" aria-hidden="true">${hint}</span>` : ""}
                </div>
                ${value
                  ? `<div class="account-table-head-total mono ${column.key === "ad_name" ? "is-account-total" : "is-metric-total"}">${value}</div>`
                  : `<div class="account-table-head-total is-empty" aria-hidden="true"></div>`}
              </div>
            </th>
          `;
...
function parseIdJsonValues(value) {
  ...
  try {
    ...
    return parsed.map((item) => Number(item || 0)).filter((item) => item > 0);
  } catch {
    return [];
  }
}

function buildMaterialTableSummary(rows, mode = MATERIAL_VIEW_PERFORMANCE) {
  const normalizedMode = normalizeMaterialViewMode(mode);
  const planIds = new Set();
  const advertiserIds = new Set();
  const summary = {
    material_mode: normalizedMode,
  ...
  rows.forEach((row) => {
    ...
    summary.order_count += Number(row?.order_count || 0);
    summary.settled_order_count += Number(row?.settled_order_count || 0);
    summary.overall_show_count += Number(row?.overall_show_count || 0);
    summary.overall_click_count += Number(row?.overall_click_count || 0);
    if (normalizedMode === MATERIAL_VIEW_RELATION) {
      const advertiserId = Number(row?.advertiser_id || 0);
      const adId = Number(row?.ad_id || 0);
      if (advertiserId > 0) advertiserIds.add(advertiserId);
      if (adId > 0) planIds.add(adId);
      return;
    }
    parseIdJsonValues(row?.plan_ids_json).forEach((item) => planIds.add(item));
    parseIdJsonValues(row?.advertiser_ids_json).forEach((item) => advertiserIds.add(item));
    const advertiserId = Number(row?.advertiser_id || 0);
    const adId = Number(row?.ad_id || 0);
    if (advertiserId > 0) advertiserIds.add(advertiserId);
    if (adId > 0) planIds.add(adId);
  ...
  summary.total_pay_amount = Number(summary.total_pay_amount.toFixed(2));
  summary.settled_pay_amount = Number(summary.settled_pay_amount.toFixed(2));
  summary.overall_ctr = summary.overall_show_count > 0
    ? Number((summary.overall_click_count / summary.overall_show_count * 100).toFixed(2))
    : 0;
  summary.roi = summary.stat_cost > 0 ? Number((summary.pay_amount / summary.stat_cost).toFixed(2)) : 0;
  summary.settled_roi = summary.stat_cost > 0 ? Number((summary.settled_pay_amount / summary.stat_cost).toFixed(2)) : 0;
  summary.plan_count = planIds.size;
  summary.advertiser_count = advertiserIds.size;
  summary.summary_text = normalizedMode === MATERIAL_VIEW_RELATION
    ? `共 ${formatNumber(summary.material_count)} 条关系行`
    : normalizedMode === MATERIAL_VIEW_LIBRARY
      ? `共 ${formatNumber(summary.material_count)} 条素材资产`
      : `共 ${formatNumber(summary.material_count)} 条素材`;
  return summary;
}

function formatMaterialHeaderValue(columnKey, summary, mode = MATERIAL_VIEW_PERFORMANCE) {
  const normalizedMode = normalizeMaterialViewMode(mode);
  switch (columnKey) {
    case "overall_ctr":
      return formatPercent(summary.overall_ctr);
    case "stat_cost":
    case "pay_amount":
      return formatMoney(summary[columnKey]);
    case "total_pay_amount":
      return normalizedMode === MATERIAL_VIEW_LIBRARY ? "" : formatMoney(summary.total_pay_amount);
    case "settled_pay_amount":
      return normalizedMode === MATERIAL_VIEW_LIBRARY ? "" : formatMoney(summary.settled_pay_amount);
    case "roi":
      return normalizedMode === MATERIAL_VIEW_LIBRARY ? "" : formatRate(summary.roi);
    case "order_count":
      return normalizedMode === MATERIAL_VIEW_LIBRARY ? "" : formatNumber(summary.order_count);
    case "plan_count":
...
function makeMaterialHeader(columns, sortState, summaryText, summaryMetrics, mode = MATERIAL_VIEW_PERFORMANCE) {
  return `
    <thead>
      <tr>
        ${columns.map((column) => {
          ...
          return `
            <th data-key="${column.key}" class="account-table-th ${column.sortable ? "is-sortable" : ""} ${columnClass}">
              <div class="account-table-head ${column.key === "material_name" ? "is-primary" : ""}">
                ...
                ${value
                  ? `<div class="account-table-head-total mono ${column.key === "material_name" ? "is-account-total" : "is-metric-total"}">${value}</div>`
                  : `<div class="account-table-head-total is-empty" aria-hidden="true"></div>`}
              </div>
            </th>
          `;
...
function renderAccountTable(accounts) {
  ...
  const enrichedRows = accounts.map((row) => enrichAccountRow(row, new Map()));
  ...
  const summaryText = query && sorted.length !== enrichedRows.length
    ? `匹配 ${formatNumber(sorted.length)} / 共 ${formatNumber(enrichedRows.length)} 个账户`
    : `共 ${formatNumber(sorted.length)} 个账户`;
  const summaryMetrics = buildAccountTableSummary(sorted);

  accountTable.innerHTML = `
    ${makeAccountHeader(columns, activeSort, summaryText, summaryMetrics)}
    <tbody>
      ${sorted.map((row) => `
        <tr>
          <td class="account-matrix-account-cell">
            <div class="cell-primary account-matrix-name">${escapeHtml(row.advertiser_name)}</div>
            <div class="cell-subline mono account-matrix-meta">
          ...
          <td class="mono account-metric-cell"><span class="account-metric-value">${formatMoney(row.stat_cost)}</span></td>
          <td class="mono account-metric-cell"><span class="account-metric-value">${formatMoney(row.pay_amount)}</span></td>
          <td class="mono account-metric-cell"><span class="account-metric-value">${formatMoney(row.total_pay_amount)}</span></td>
          <td class="mono account-metric-cell"><span class="account-metric-value">${formatMoney(row.settled_pay_amount)}</span></td>
          <td class="mono account-metric-cell"><span class="account-metric-value">${formatRate(row.roi)}</span></td>
          <td class="mono account-metric-cell"><span class="account-metric-value">${formatRate(row.settled_roi)}</span></td>
          <td class="mono account-metric-cell"><span class="account-metric-value">${formatNumber(row.order_count)}</span></td>
          <td class="mono account-metric-cell"><span class="account-metric-value">${formatNumber(row.settled_order_count)}</span></td>
          <td class="mono account-metric-cell"><span class="account-metric-value">${Number(row.order_count || 0) > 0 ? formatMoney(row.pay_order_cost) : "-"}</span></td>
          <td class="mono account-metric-cell"><span class="account-metric-value">${Number(row.total_pay_amount || 0) > 0 ? formatPercent(row.settled_amount_rate) : "-"}</span></td>
          <td class="mono account-metric-cell"><span class="account-metric-value">${Number(row.total_pay_amount || 0) > 0 ? formatPercent(row.refund_rate_1h) : "-"}</span></td>
          <td class="mono account-metric-cell">
            ${Number(row.plan_count || 0) > 0
              ? `<button type="button" class="relation-trigger account-plan-count-trigger mono" data-action="open-account-plan-detail" data-advertiser-id="${row.advertiser_id}">${formatNumber(row.plan_count)}</button>`
              : `<span class="account-metric-value">${formatNumber(row.plan_count)}</span>`}
          </td>
          <td class="account-status-cell"><span class="${accountStatusClass(row.status_text)}">${escapeHtml(row.status_text)}</span></td>
        </tr>
      `).join("")}
    </tbody>
  `;
  ...
  accountTable.querySelectorAll('[data-action="open-account-plan-detail"]').forEach((button) => {
    ...
    button.addEventListener("click", async (event) => {
      ...
      await openAccountPlanRelationDetail(row);
    });
  });
}
...
function renderMaterialDetail(materialKey) {
  if (!materialDetail) return;
  const payload = materialRangePayload(materialFilter()) || {};
  const rows = payload?.items || [];
  const targetKey = String(materialKey || "").trim();
  const sourceRow = rows.find((item) => materialRowIdentity(item) === targetKey);
  ...
  const summaryText = relationMode
    ? `当前关系行消耗 ${formatMoney(row.stat_cost)}，支付金额 ${formatMoney(row.pay_amount)}，归属于 ${escapeHtml(row.advertiser_name || row.top_account_name || "-")} / ${escapeHtml(row.ad_name || row.top_plan_name || "-")}。该视图只有保持“素材 x 账户 x 计划”关系粒度时，才能用于与账户或计划总额核对。`
    : (libraryMode
  ...
  materialDetail.innerHTML = `
    <div class="detail-shell-head">
      <div class="detail-shell-copy">
        <span class="detail-eyebrow">${relationMode ? "素材关系核对" : (libraryMode ? "素材资产详情" : "素材表现详情")}</span>
        ...
        <div class="detail-meta-row">
          ...
          ${relationMode
            ? `${detailMetaPill(`账户 ${escapeHtml(row.advertiser_name || row.top_account_name || "--")}`)}${detailMetaPill(`计划 ${escapeHtml(row.ad_name || row.top_plan_name || "--")}`)}`
            : `${detailMetaPill(`覆盖账户 ${formatNumber(row.advertiser_count)}`)}${detailMetaPill(`覆盖计划 ${formatNumber(row.plan_count)}`)}`}
        </div>
      </div>
      <div class="detail-actions">
        ${canPreviewMaterial(row)
          ? `<button type="button" class="button ghost compact" data-action="open-material-preview" data-material-key="${escapeHtml(materialRowIdentity(row))}">查看预览</button>`
          : ""}
      </div>
    </div>
    <div class="detail-highlight-grid">
      ...
      ${detailHighlightCard("归属账户", escapeHtml(row.advertiser_name || row.top_account_name || "--"), "compact")}
      ${detailHighlightCard("归属计划", escapeHtml(row.ad_name || row.top_plan_name || "--"), "compact")}
      `
        : (libraryMode
        ? `
      ${detailHighlightCard("创建时间", escapeHtml(formatDateTime(row.create_time || "")), "mono")}
      ${detailHighlightCard("覆盖账户", formatNumber(row.advertiser_count), "mono")}
      ${detailHighlightCard("覆盖计划", formatNumber(row.plan_count), "mono")}
      ${detailHighlightCard("素材类型", escapeHtml(row.material_type || "--"), "mono")}
      `
        : `
      ${detailHighlightCard("消耗", formatMoney(row.stat_cost), "mono")}
      ${detailHighlightCard("整体成交", formatMaterialTotalPayAmount(row), "mono")}
    ...
    <div class="detail-grid-section">
      <div class="detail-section-title">${relationMode ? "核对信息" : (libraryMode ? "资产信息" : "效果拆分")}</div>
      <div class="detail-metric-grid">
        ...
        ${detailMetricCard("首次发现", escapeHtml(formatDateTime(row.first_seen_at || "") || "--"), "compact mono")}
        ${detailMetricCard("最近发现", escapeHtml(formatDateTime(row.last_seen_at || "") || "--"), "compact mono")}
        `
          : (libraryMode
          ? `
        ${detailMetricCard("素材标记", row.is_original ? "首发素材" : "常规素材", "compact")}
        ${detailMetricCard("商品信息", escapeHtml(row.product_info_text || "-"), "compact")}
        ${detailMetricCard("归属账户", escapeHtml(row.top_account_name || "-"), "compact")}
        ${detailMetricCard("归属计划", escapeHtml(row.top_plan_name || "-"), "compact")}
        ${detailMetricCard("达人", escapeHtml(row.top_anchor_name || "-"), "compact")}
    ...
    <div class="detail-grid-section">
      ...
      <div class="detail-metric-grid">
        ...
        ${detailMetricCard(relationMode ? "归属账户" : "代表账户", escapeHtml((relationMode ? row.advertiser_name : row.top_account_name) || row.top_account_name || "-"), "compact")}
        ${detailMetricCard(relationMode ? "归属计划" : "代表计划", escapeHtml((relationMode ? row.ad_name : row.top_plan_name) || row.top_plan_name || "-"), "compact")}
  ...
  materialDetail.querySelector('[data-action="open-material-preview"]')?.addEventListener("click", () => {
    openMaterialPreview(materialRowIdentity(row));
  });
}
...
function syncSelectedMaterial(rows) {
  ...
  const exists = rows.some((item) => materialRowIdentity(item) === state.selectedMaterialKey);
  if (!exists) {
  ...
  materialTablePager.querySelectorAll("button[data-page]").forEach((button) => {
    button.addEventListener("click", async () => {
      ...
      try {
        ...
        window.alert(error.message || "素材分页加载失败");
      }
    });
  });
}

function bindMaterialPreviewInteractions(container) {
  if (!container) return;
  container.querySelectorAll('[data-action="open-material-preview"]').forEach((button) => {
    button.addEventListener("click", (event) => {
      event.stopPropagation();
  ...
  container.querySelectorAll(".material-preview-thumb").forEach((thumb) => {
  ...
  if (materialDetail) {
    materialTable.querySelectorAll("tbody tr").forEach((rowEl) => {
      rowEl.addEventListener("click", () => {
        ...
        renderMaterialTable(materialRangePayload(materialFilter()) || { items: rows });
      });
    });
  }
  bindMaterialPreviewInteractions(materialTable);
}

function renderMaterialTable(payload) {
  const operatorMode = isOperator();
  ...
  const allowedSortKeys = relationMode
    ? ["material_name", "create_time", "overall_ctr", "stat_cost", "total_pay_amount", "settled_pay_amount", "roi", "pay_amount", "order_count", "advertiser_name", "ad_name", "top_anchor_name", "first_seen_at", "last_seen_at", "seen_count", "active_day_count"]
    : (libraryMode
      ? (operatorMode
        ? ["material_name", "create_time", "top_anchor_name", "plan_count"]
        : ["material_name", "create_time", "top_account_name", "top_plan_name", "top_anchor_name", "plan_count", "advertiser_count"])
      : (operatorMode
        ? ["material_name", "create_time", "overall_ctr", "stat_cost", "top_anchor_name"]
        : ["material_name", "create_time", "overall_ctr", "stat_cost", "total_pay_amount", "settled_pay_amount", "roi", "pay_amount", "order_count", "top_anchor_name", "plan_count", "advertiser_count"]));
  if (!allowedSortKeys.includes(String(state.materialSort.key || ""))) {
    state.materialSort = libraryMode ? { key: "create_time", dir: "desc" } : { key: "stat_cost", dir: "desc" };
    saveSort("material-sort", state.materialSort);
  }
  const columns = relationMode
    ? [
        ...
        { key: "first_seen_at", label: "首次发现", sortable: true },
        { key: "last_seen_at", label: "最近发现", sortable: true },
      ]
    : (libraryMode
      ? (operatorMode
        ? [
  ...
  const pageStart = Number(pagination.start_index || (totalRows > 0 ? 1 : 0));
  const pageEnd = Number(pagination.end_index || enrichedRows.length || 0);
  const pageRows = enrichedRows;
  const summaryMetrics = normalizedPayload.materials_aggregate || buildMaterialTableSummary(enrichedRows, mode);
  let summaryText = String(summaryMetrics?.summary_text || "").trim() || (
    relationMode
      ? `共 ${formatNumber(totalRows)} 条关系行`
      : libraryMode
        ? `共 ${formatNumber(totalRows)} 条素材资产`
        : `共 ${formatNumber(totalRows)} 条素材`
  );
  if (libraryMode && normalizedPayload?.meta?.create_time_fallback) {
    summaryText = `${summaryText} · 创建时间待补`;
  }
        ...
        <tr data-material-key="${escapeHtml(rowKey)}" class="${supportsMaterialDetail && state.selectedMaterialKey === rowKey ? "active-row" : ""}">
          <td>
            ...
            <div class="cell-subline mono">
              ...
              <span class="cell-subitem">${escapeHtml(row.material_type || "OTHER")}</span>
              ${relationMode ? `<span class="cell-subitem">活跃 ${formatNumber(row.active_day_count || 0)} 天</span>` : ""}
            </div>
          </td>
          <td>
          ...
          <td>
            ${materialPreviewTriggerMarkup(row)}
          </td>
          <td>${renderDateTimeCell(row.create_time || "")}</td>
          ${relationMode
            ? `
          ...
          <td class="mono">${formatMoney(row.pay_amount)}</td>
          <td class="mono">${formatNumber(row.order_count)}</td>
          <td>${escapeHtml(row.advertiser_name || row.top_account_name || "--")}</td>
          <td>${escapeHtml(row.ad_name || row.top_plan_name || "--")}</td>
          <td>${escapeHtml(row.top_anchor_name || "--")}</td>
          <td>${renderDateTimeCell(row.first_seen_at || "", "--")}</td>
          <td>${renderDateTimeCell(row.last_seen_at || "", "--")}</td>
          `
            : (libraryMode
            ? (operatorMode
              ? `
          ...
          <td class="mono">
            ${Number(row.plan_count || 0) > 0
              ? `<button type="button" class="relation-trigger mono" data-action="open-material-plan-detail" data-material-key="${escapeHtml(rowKey)}">${formatNumber(row.plan_count)}</button>`
              : formatNumber(row.plan_count)}
          </td>
          `
              : `
          ...
          <td class="mono">
            ${Number(row.advertiser_count || 0) > 0
              ? `<button type="button" class="relation-trigger mono" data-action="open-material-account-detail" data-material-key="${escapeHtml(rowKey)}">${formatNumber(row.advertiser_count)}</button>`
              : formatNumber(row.advertiser_count)}
          </td>
          `)
            : `
  ...
  materialTable.querySelectorAll("th[data-key]").forEach((header) => {
    header.addEventListener("click", () => {
      ...
      refreshMaterialSection(false).catch((error) => {
        window.alert(error.message || "素材排序刷新失败");
      });
    });
  });
  materialTable.querySelectorAll('[data-action="open-material-plan-detail"], [data-action="open-material-account-detail"]').forEach((button) => {
    const materialKey = String(button.dataset.materialKey || "");
    const row = pageRows.find((item) => materialRowIdentity(item) === materialKey);
    if (!row) return;
    button.addEventListener("click", (event) => {
      event.stopPropagation();
  ...
  renderMaterialInteractions(pageRows);
}
  ...
  teamMaterialTablePager.querySelectorAll("button[data-page]").forEach((button) => {
    button.addEventListener("click", async () => {
      ...
      try {
        ...
        window.alert(error.message || "团队素材分页加载失败");
      }
    });
  });
}

function renderTeamMaterialTable(payload) {
...
function renderTeamMaterialInteractions(rows) {
  if (!teamMaterialTable) return;
  bindMaterialPreviewInteractions(teamMaterialTable);
}
...
async function openAccountPlanRelationDetail(row) {
  const payload = rangePayload(sectionFilter("account"));
  const advertiserId = Number(row?.advertiser_id || 0);
  const planFilter = {
    ...normalizeRangeFilter(sectionFilter("account")),
    section: "plan",
  };
  const planPayload = await fetchPerformance(planFilter, false).catch(() => rangePayload(planFilter));
  const plans = buildPlanRelationItems((planPayload?.plans || []).filter((item) => Number(item?.advertiser_id || 0) === advertiserId));
...
async function openMaterialRelationDetail(row, kind) {
  const payload = materialRangePayload(materialFilter());
  ...
  try {
    ...
    if (requestToken !== state.materialRelationDetailRequestToken) return;
    const mergedItems = kind === "account"
      ? mergeMaterialAccountRelationItems(localItems, resolvedPayload?.accounts || [])
      : mergeMaterialPlanRelationItems(localItems, resolvedPayload?.plans || []);
    openRelationDetailModal({
      title,
      meta,
      kind,
...
async function fetchMaterialRankings(force = false) {
  const filter = materialFilter();
  const requestState = materialRequestState();
  const cacheKey = materialPayloadKey(filter, requestState);
  const cachedPayload = state.materialPayloads[cacheKey];
  if (!force && cachedPayload) {
    const expectedSnapshot = materialCacheSnapshotToken(filter);
    ...
    const ttlMs = materialCacheTtlMs(filter);
    const freshBySnapshot = Boolean(expectedSnapshot) && cachedSnapshot === expectedSnapshot;
    const freshByAge = cachedAt > 0 && Date.now() - cachedAt < ttlMs;
    if (freshBySnapshot || freshByAge) {
      return cachedPayload;
    }
  }
  const params = appendDisplayScopeParam(new URLSearchParams());
  const endpoint = "/api/material-rankings";
  params.set("range", filter.mode);
  params.set("mode", filter.materialMode);
  ...
  if (state.materialRequestPromises[cacheKey]) {
    return state.materialRequestPromises[cacheKey];
  }
  const requestPromise = (async () => {
    const response = await fetch(`${endpoint}?${params.toString()}`).catch(() => null);
    if (!response || !response.ok) {
      const errorPayload = response ? await response.json().catch(() => ({})) : {};
      if (cachedPayload) {
        return cachedPayload;
...
async function fetchTeamMaterialRankings(force = false) {
  ...
  const requestState = teamMaterialRequestState();
  const cacheKey = teamMaterialPayloadKey(filter, requestState);
  ...
  if (state.teamMaterialRequestPromises[cacheKey]) {
    return state.teamMaterialRequestPromises[cacheKey];
  }
  const requestPromise = (async () => {
  ...
  state.teamMaterialRequestPromises[cacheKey] = requestPromise;
  try {
    return await requestPromise;
  } finally {
...
function renderPlanTable(plans) {
  ...
  const sortedRows = sortRows(rows, state.planSort);
  const summaryText = (query || accountFilter)
    ? `匹配 ${formatNumber(sortedRows.length)} / 共 ${formatNumber(enrichedRows.length)} 个计划`
    : `共 ${formatNumber(sortedRows.length)} 个计划`;
  const summaryMetrics = buildPlanTableSummary(sortedRows);
  const supportsPlanDetail = Boolean(planDetail && planAssetSummary);

  planTable.innerHTML = `
    ${makePlanHeader(columns, state.planSort, summaryText, summaryMetrics)}
    <tbody>
      ${sortedRows.map((row) => `
        <tr data-plan-id="${row.ad_id}" class="${supportsPlanDetail && state.selectedPlanId === row.ad_id ? "active-row" : ""}">
          <td>
...
function selectedMaterialRow(materialKey) {
  ...
  if (!key) return null;
  return currentMaterialRows().find((item) => materialRowIdentity(item) === key)
    || currentTeamMaterialRows().find((item) => materialRowIdentity(item) === key)
    || null;
}
...
function materialPreviewTriggerMarkup(row) {
  ...
  const materialKey = escapeHtml(materialRowIdentity(row));
...
function previewUrlExpiresAt(url) {
...
function previewUrlNeedsRefresh(url, leewaySeconds = 0) {
  const text = String(url || "").trim();
  if (!text) return true;
  const expiresAt = previewUrlExpiresAt(text);
  if (expiresAt <= 0) return false;
  return expiresAt <= Math.floor(Date.now() / 1000) + Math.max(Number(leewaySeconds) || 0, 0);
}
...
function isLikelyDirectPreviewCoverUrl(url) {
  ...
  try {
    ...
    if (host.endsWith("creativityeco.com")) {
      return !previewUrlNeedsRefresh(text, 0);
    }
...
const PREVIEW_MEDIA_FALLBACK_TIMEOUT_MS = 2500;

function replaceMaterialPreviewMediaShell(previewMediaShell, previewCurvePanel, markup) {
  if (!previewMediaShell) return;
  previewMediaShell.innerHTML = markup;
  if (previewCurvePanel) {
    previewMediaShell.appendChild(previewCurvePanel);
  }
}

function renderMaterialPreviewFallback(previewMediaShell, previewCurvePanel, row, options = {}) {
  const coverUrl = String(options.coverUrl ?? row?.cover_url ?? "").trim();
  const message = String(options.message || "").trim();
  replaceMaterialPreviewMediaShell(
    previewMediaShell,
    previewCurvePanel,
    materialPreviewMediaStageMarkup(row, { coverUrl, message }),
  );
  ...
  fallbackCover.addEventListener("error", () => {
    replaceMaterialPreviewMediaShell(
      previewMediaShell,
      previewCurvePanel,
      '<div class="preview-media-stage"><div class="preview-empty">当前素材没有可站外访问的预览地址，请尝试打开抖音作品。</div></div>',
    );
  }, { once: true });
}

function bindMaterialPreviewMediaState(previewMediaShell, previewCurvePanel, row, options = {}) {
  if (!previewMediaShell) return;
  ...
  const coverUrl = String(options.coverUrl ?? row?.cover_url ?? "").trim();
  const fallbackMessage = String(options.fallbackMessage || "").trim() || "当前视频地址无法直接播放，已降级为封面预览。";
  const emptyMessage = String(options.emptyMessage || "").trim() || "当前素材没有可站外访问的预览地址，请尝试打开抖音作品。";
  if (previewVideo) {
    let settled = false;
    let fallbackTimer = 0;
    const settle = () => {
    ...
    const fallbackToCover = () => {
      if (settled) return;
      settle();
      renderMaterialPreviewFallback(
        previewMediaShell,
        previewCurvePanel,
        row,
        {
    ...
    previewVideo.addEventListener("loadedmetadata", () => {
      ...
      if (Number(previewVideo.readyState || 0) >= HTMLMediaElement.HAVE_METADATA) {
        settle();
      }
    }, { once: true });
    previewVideo.addEventListener("loadeddata", settle, { once: true });
    previewVideo.addEventListener("canplay", settle, { once: true });
    previewVideo.addEventListener("playing", () => {
      stabilizePreviewVideoElement(previewVideo);
      settle();
    }, { once: true });
    previewVideo.addEventListener("error", fallbackToCover, { once: true });
    fallbackTimer = window.setTimeout(() => {
      if (settled) return;
      if (!materialPreviewModal || materialPreviewModal.classList.contains("hidden")) {
        settle();
        return;
      }
      const activeSrc = String(previewVideo.currentSrc || previewVideo.getAttribute("src") || "").trim();
      const readyState = Number(previewVideo.readyState || 0);
      const networkState = Number(previewVideo.networkState || 0);
      const noSource = networkState === HTMLMediaElement.NETWORK_NO_SOURCE;
      if (!activeSrc) {
        settle();
        return;
      }
      if (previewVideo.error || noSource || readyState === 0) {
        fallbackToCover();
      }
    }, PREVIEW_MEDIA_FALLBACK_TIMEOUT_MS);
    return;
  }
...
async function hydrateMaterialPreviewSource(row) {
  ...
  const materialKey = materialRowIdentity(row);
  if (!materialKey) return;
  ...
  try {
    ...
    bindMaterialPreviewMediaState(
      previewMediaShell,
      previewCurvePanel,
      mergedRow,
      {
        coverUrl: mergedRow.cover_url,
        fallbackMessage: resolved.reason || "已解析到视频地址，但当前浏览器仍无法直接播放。",
        emptyMessage: resolved.reason || "当前素材没有可站外访问的预览地址，请尝试打开抖音作品。",
      },
    );
  } catch {
    if (!materialPreviewBody || materialPreviewBody.dataset.materialKey !== materialKey) return;
...
function materialPreviewCurveRequest(row) {
  const filter = activeMaterialPreviewFilter();
  const params = appendDisplayScopeParam(new URLSearchParams());
...
async function loadMaterialPreviewCurve(row) {
  ...
  if (activeMaterialPreviewMode() === MATERIAL_VIEW_LIBRARY) {
    panel.innerHTML = materialPreviewCurveEmptyMarkup("资产库模式不展示阶段曲线", "切回“表现”后可查看当前时间范围内的视频互动峰形。");
    return;
  }
...
function openMaterialPreviewFromRow(row) {
  ...
  const operatorMode = isOperator();
  const previewMode = activeMaterialPreviewMode();
  const libraryMode = previewMode === MATERIAL_VIEW_LIBRARY;
  const relationMode = previewMode === MATERIAL_VIEW_RELATION;
  row = enrichMaterialRow(row);
  const rowIdentity = materialRowIdentity(row);
  ...
  if (materialPreviewMeta) {
    materialPreviewMeta.textContent = [
      ...(operatorMode ? [] : [relationMode ? (row.advertiser_name || row.top_account_name || "") : (row.top_account_name || ""), relationMode ? (row.ad_name || row.top_plan_name || "") : (row.top_plan_name || "")]),
      row.top_anchor_name || "",
  ...
  const previewBlock = materialPreviewMediaStageMarkup(
    row,
    {
      ...
      coverUrl,
      message: safeDirectVideoUrl || coverUrl
        ? ""
  ...
  const detailStatsMarkup = operatorMode
    ? (libraryMode
      ...
      <div class="preview-stat"><span>创建时间</span><strong class="mono">${escapeHtml(formatDateTime(row.create_time || ""))}</strong></div>
      <div class="preview-stat"><span>覆盖计划</span><strong class="mono">${formatNumber(row.plan_count)}</strong></div>
      `
      : (relationMode
        ? `
      <div class="preview-stat"><span>达人</span><strong>${escapeHtml(row.top_anchor_name || "--")}</strong></div>
      <div class="preview-stat"><span>归属账户</span><strong>${escapeHtml(row.advertiser_name || row.top_account_name || "--")}</strong></div>
      <div class="preview-stat"><span>归属计划</span><strong>${escapeHtml(row.ad_name || row.top_plan_name || "--")}</strong></div>
      ...
      <div class="preview-stat"><span>最近发现</span><strong class="mono">${escapeHtml(formatDateTime(row.last_seen_at || "") || "--")}</strong></div>
    `
      : `
      <div class="preview-stat"><span>达人</span><strong>${escapeHtml(row.top_anchor_name || "--")}</strong></div>
      <div class="preview-stat"><span>消耗</span><strong class="mono">${formatMoney(row.stat_cost)}</strong></div>
    ...
    : (libraryMode
      ...
      <div class="preview-stat"><span>创建时间</span><strong class="mono">${escapeHtml(formatDateTime(row.create_time || ""))}</strong></div>
      <div class="preview-stat"><span>覆盖计划</span><strong class="mono">${formatNumber(row.plan_count)}</strong></div>
      <div class="preview-stat"><span>覆盖账户</span><strong class="mono">${formatNumber(row.advertiser_count)}</strong></div>
    ...
    `
      ...
      <div class="preview-stat"><span>首次发现</span><strong class="mono">${escapeHtml(formatDateTime(row.first_seen_at || "") || "--")}</strong></div>
      <div class="preview-stat"><span>最近发现</span><strong class="mono">${escapeHtml(formatDateTime(row.last_seen_at || "") || "--")}</strong></div>
      <div class="preview-stat"><span>关系活跃天数</span><strong class="mono">${formatNumber(row.active_day_count)}</strong></div>
      <div class="preview-stat"><span>素材 ID</span><strong class="mono">${escapeHtml(row.material_id || "--")}</strong></div>
      <div class="preview-stat"><span>视频 ID</span><strong class="mono">${escapeHtml(row.video_id || "--")}</strong></div>
    `
      : `
      ...
      <div class="preview-stat"><span>视频 ID</span><strong class="mono">${escapeHtml(row.video_id || "--")}</strong></div>
    `));
  const previewCurveMarkup = libraryMode
    ? materialPreviewCurveEmptyMarkup("资产库模式不展示阶段曲线", "切回“表现”后可查看当前时间范围内的视频互动峰形。")
    : materialPreviewCurveLoadingMarkup();
  materialPreviewBody.innerHTML = `
    <div class="preview-media-shell">
      ${previewBlock}
      <div class="preview-curve-panel" data-role="preview-curve-panel">${previewCurveMarkup}</div>
    </div>
  ...
  bindMaterialPreviewMediaState(
    ...
    row,
    {
      ...
      fallbackMessage: "当前视频地址无法直接播放，已降级为封面预览。",
      emptyMessage: "当前素材没有可站外访问的预览地址，请尝试打开抖音作品。",
    },
  );
  materialPreviewModal.classList.remove("hidden");
  materialPreviewModal.setAttribute("aria-hidden", "false");
  materialPreviewBody.dataset.materialKey = rowIdentity;
        ...
        <tr>
          ...
          <td>
            <button
              ...
              data-material-key="${escapeHtml(materialRowIdentity(item))}"
            >预览</button>
          </td>
          <td class="mono">${formatMoney(item.stat_cost)}</td>
...
function isEditingSelectedUser() {
  return state.userEditorMode !== "create" && Number(state.selectedUserId || 0) > 0;
}
  ...
  userTable.innerHTML = `
    ...
    <tbody>
      ${state.users.length ? state.users.map((item) => `
        <tr data-user-id="${item.id}" class="${isEditingSelectedUser() && Number(state.selectedUserId) === Number(item.id) ? "active-row" : ""}">
          <td>
...
async function fetchUsers(force = false) {
  ...
  if (!state.selectedUserId && state.userEditorMode !== "create" && state.users.length) {
...
async function fetchPerformance(filter, force = false) {
  const normalized = {
    ...normalizeRangeFilter(filter),
    section: String(filter?.section || "all").trim() || "all",
  };
...
function renderPerformanceSection(sectionKey) {
  ...
  if (normalizedSectionKey === "plan") {
    ...
    fillPlanAccountFilter((planPayload?.plan_accounts || planPayload?.accounts || []).map((row) => row.advertiser_name));
...
function renderPerformanceSections(sectionKeys = ["account", "plan", "breakdown"]) {
  sectionKeys.forEach((sectionKey) => {
    renderPerformanceSection(sectionKey);
  });
}
...
async function refreshPerformanceSections(force = false) {
  ...
  renderPerformanceSections(["account", "plan", "breakdown"]);
}
...
async function refreshPerformanceSection(sectionKey, force = false) {
  ...
  renderPerformanceSection(normalizedSectionKey);
}
...
async function refreshMaterialSection(force = false) {
  ...
  renderMaterialTable(payload || {});
  syncSelectedMaterial(payload?.items || []);
  const meta = payload?.meta || {};
  const materialTodayStatus = payload?.materialTodayStatus || state.payload?.materialSync?.materialTodayStatus || state.payload?.latest?.materialTodayStatus || {};
  const rangeText = formatDateWindowMeta(payload);
  const materialCount = Number(
    payload?.pagination?.total_count
    || meta.material_count
    || meta.relation_count
    || meta.material_row_count
    || (Array.isArray(payload?.items) ? payload.items.length : 0),
  );
  const mode = normalizeMaterialViewMode(payload?.material_mode || materialViewMode());
  const todaySuffix = mode !== MATERIAL_VIEW_LIBRARY
    ? formatMaterialTodayStatusSuffix(materialTodayStatus)
    : "";
  const syncText = mode === MATERIAL_VIEW_LIBRARY
    ? (
      payload?.snapshot_time
        ? `${rangeText} · 资产库素材 ${formatNumber(materialCount)} 条 · 最近资产更新 ${payload.snapshot_time}`
        : `${rangeText} · 当前时间范围内暂无素材资产`
    )
    : (mode === MATERIAL_VIEW_RELATION
      ? (
        payload?.snapshot_time
          ? `${rangeText} · 关系行 ${formatNumber(materialCount)} 条 · 最近关系快照 ${payload.snapshot_time}${todaySuffix}`
          : `${rangeText} · 当前时间范围内暂无素材关系${todaySuffix}`
      )
      : (
      payload?.snapshot_time
  ...
  if (materialSemanticsNotice) {
    ...
    if (noticeState.text) {
      materialSemanticsNotice.textContent = noticeState.text;
      materialSemanticsNotice.dataset.tone = noticeState.tone || "neutral";
      materialSemanticsNotice.classList.remove("hidden");
    } else {
      materialSemanticsNotice.textContent = "";
      materialSemanticsNotice.classList.add("hidden");
    }
  }
}
...
async function refreshTeamMaterialSection(force = false) {
  ...
  renderTeamMaterialTable(payload || {});
  const meta = payload?.meta || {};
  const rangeText = formatDateWindowMeta(payload);
  const materialCount = Number(
    payload?.materials_aggregate?.material_count
    || payload?.pagination?.total_count
    || (Array.isArray(payload?.items) ? payload.items.length : 0)
    || meta.material_count
    || meta.material_row_count
  ...
  const syncText = payload?.snapshot_time
    ? `${rangeText} · 汇总 ${formatNumber(payload?.snapshot_count || 0)} 个日快照 · 最近素材热同步 ${payload.snapshot_time} · 素材 ${formatNumber(materialCount)} 条 · 错误 ${formatNumber(meta.error_count || 0)}`
...
async function queueTodayDetailRefresh() {
  const response = await fetch("/api/sync/material-hot?force_refresh=1", { method: "POST" });
  const payload = await response.json().catch(() => ({}));
  if (!response.ok) {
    throw new Error(payload?.detail || payload?.message || "今日素材热刷新提交失败");
  }
  return payload;
}

async function runTodayPageRefresh(button, loadingText, callback) {
  const idleText = button?.dataset?.idleText || button?.textContent || "刷新当前页";
  if (button) {
...
function schedulePerformanceRangeWarmup(sectionKey = "") {
  ...
  if (activeFilter.mode !== "day") return;
  const warmupMode = "week";
  const runWarmup = () => {
    fetchPerformance({ mode: warmupMode, section: normalizedSectionKey }, false).catch(() => {});
  };
...
function bindInputs() {
  ...
  materialModeSwitch?.querySelectorAll(".range-button").forEach((button) => {
    button.addEventListener("click", async () => {
      ...
      if (nextMode === materialViewMode()) return;
      state.materialViewMode = nextMode;
      savePreference(MATERIAL_VIEW_MODE_KEY, nextMode);
      state.materialSort = nextMode === MATERIAL_VIEW_LIBRARY ? { key: "create_time", dir: "desc" } : { key: "stat_cost", dir: "desc" };
      saveSort("material-sort", state.materialSort);
      state.materialPage = 1;
      syncMaterialViewModeControls();
      try {
        ...
        window.alert(error.message || "切换素材模式失败");
      }
    });
  });
  ...
  syncButton?.addEventListener("click", async () => {
    const confirmed = window.confirm(
      [
        "将执行页面回填。",
        "这会先清理页面缓存，再按数据库最新快照回填页面。",
        "不会主动重新采集全部接口数据。",
        "页面内的“刷新当前页”按钮只会重新读取当前页面数据。",
        "",
        "确认继续吗？",
      ].join("\n"),
    );
    ...
    try {
      ...
      const payload = await response.json().catch(() => ({}));
      if (!response.ok) {
        throw new Error(payload?.detail || payload?.message || "页面回填任务提交失败");
      }
      state.fullRefreshStatus = payload?.status || state.fullRefreshStatus;
      applyFullRefreshStatus();
      scheduleFullRefreshStatusPoll();
      if (payload?.running) {
        window.alert(payload?.status?.message || "已有页面回填任务正在执行，请等待当前任务完成。");
        return;
      }
      window.setTimeout(() => {
        fetchDashboard(true).catch(() => {});
      ...
      window.alert(error.message || "页面回填任务提交失败");
    }
  });

  if (syncExtendedButton) {
    syncExtendedButton.addEventListener("click", async () => {
      await runTodayPageRefresh(syncExtendedButton, "刷新中...", async () => {
        state.materialPage = 1;
  ...
  teamMaterialRefreshButton?.addEventListener("click", async () => {
    await runTodayPageRefresh(teamMaterialRefreshButton, "刷新中...", async () => {
      state.teamMaterialPage = 1;
...
async function fetchDashboard(force = false, options = {}) {
  ...
  if (state.dashboardFetchPromise) {
    if (force) {
      ...
      state.dashboardFetchQueuedOptions = {
        ...(state.dashboardFetchQueuedOptions || {}),
        refreshMode,
      };
    }
  ...
  try {
    ...
    if (state.dashboardFetchQueued) {
      const queuedOptions = state.dashboardFetchQueuedOptions || {};
      state.dashboardFetchQueued = false;
      state.dashboardFetchQueuedOptions = null;
      window.setTimeout(() => {
        fetchDashboard(true, queuedOptions).catch(() => {});
      }, 0);
    }
  }
}
...
function fullRefreshButtonLabel(status) {
  const value = String(status?.status || "").trim().toLowerCase();
  if (value === "queued") return "页面回填排队中";
  if (value === "running") return "页面回填中";
  if (value === "failed") return "页面回填失败";
  if (value === "unknown") return "页面回填异常";
  if (value === "completed" && String(status?.message || "").includes("存在错误")) return "页面回填异常";
  return "页面回填";
}

function fullRefreshButtonTitle(status) {
  const titleParts = [];
...
function fullRefreshAuthBlockMessage(status) {
  const candidates = [];
  if (status?.message) candidates.push(String(status.message));
  const stages = status?.stages && typeof status.stages === "object" ? Object.values(status.stages) : [];
  stages.forEach((stage) => {
    if (stage?.message) candidates.push(String(stage.message));
    const errors = Array.isArray(stage?.result?.errors) ? stage.result.errors : [];
    if (errors[0]?.error) candidates.push(String(errors[0].error));
  });
  if (candidates.some((item) => item.includes("refresh_token无效"))) {
    return "鉴权失效，历史补缺已停止，需更新最新 token 后再继续。";
  }
  return "";
}
...
function applyFullRefreshStatus() {
  ...
  syncButton.title = fullRefreshButtonTitle(status);
  const authBlockedMessage = fullRefreshAuthBlockMessage(status);
  if (authBlockedMessage && historySyncCard) {
    const pillEl = historySyncCard.querySelector(".system-pill");
    const valueEl = historySyncCard.querySelector(".system-card-value");
    const copyEl = historySyncCard.querySelector(".system-card-copy");
    if (pillEl) {
      pillEl.textContent = "鉴权阻塞";
      pillEl.classList.remove("warn");
      pillEl.classList.add("danger");
    }
    if (valueEl) {
      valueEl.textContent = "历史补缺暂停";
    }
    if (copyEl) {
      copyEl.textContent = authBlockedMessage;
    }
  }
}
...
window.setInterval(() => {
  ...
  fetchDashboard(false, { refreshMode: "passive" }).catch(() => {});
}, 60 * 1000);
