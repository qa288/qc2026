...
class PerformanceAccess:
    ...
    def apply_account_scope(
        ...
        *,
        ...
        if "plan_accounts" in payload:
            next_payload["plan_accounts"] = [
                dict(item)
                for item in payload.get("plan_accounts", [])
                if int(item.get("advertiser_id", 0) or 0) in allowed
            ]
        ...
        if normalized_section in {"all", "breakdown"}:
            ...
            if normalized_section == "account" and not plans:
                next_payload["summary"]["plan_count"] = sum(int(item.get("plan_count", 0) or 0) for item in accounts)
                next_payload["summary"]["active_plan_count"] = sum(int(item.get("active_plan_count", 0) or 0) for item in accounts)
    ...
    def missing_summary_days(self, conn: Any, start_dt: datetime, end_dt: datetime) -> list[datetime]:
        ...
        current_rows = []
        if self._table_exists(conn, "summary_current"):
