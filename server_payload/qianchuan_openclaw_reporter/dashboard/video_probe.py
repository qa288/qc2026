...
def should_auto_normalize_for_upload(result: VideoProbeResult) -> bool:
    if result.coded_width <= 0 or result.coded_height <= 0:
        return False
    if result.coded_width % 2 != 0 or result.coded_height % 2 != 0:
        return True
    if int(result.rotation or 0) % 360:
        return True
    sample_aspect_ratio = str(result.sample_aspect_ratio or "").strip()
...
def normalized_upload_dimensions(result: VideoProbeResult) -> tuple[int, int]:
    width = int(result.display_width or result.coded_width or 0)
    height = int(result.display_height or result.coded_height or 0)
    if width <= 0 or height <= 0:
        raise VideoProbeError("cannot determine normalized dimensions")
    if width % 2 != 0:
        width += 1
    if height % 2 != 0:
        height += 1
...
def transcode_video_for_upload(
    input_path: Path,
    output_path: Path,
    ...
    if not executable:
        raise VideoProbeError("ffmpeg is not installed")
    target_width, target_height = normalized_upload_dimensions(probe_result)
    output_path.parent.mkdir(parents=True, exist_ok=True)
    filter_graph = f"scale={target_width}:{target_height}:flags=lanczos,setsar=1:1"
    command = [
        executable,
        "-y",
        "-hide_banner",
    ...
    try:
        ...
        if isinstance(exc, subprocess.CalledProcessError):
            ...
            detail = str(exc.stderr or exc.stdout or "").strip()
        if detail:
            raise VideoProbeError(f"ffmpeg transcode failed: {detail}") from exc
        raise VideoProbeError(f"ffmpeg transcode failed: {exc}") from exc
    if not output_path.exists() or output_path.stat().st_size <= 0:
        raise VideoProbeError("ffmpeg transcode produced no output")
