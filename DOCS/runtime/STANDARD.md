...
- 素材列表支持预览入口：
  - 有视频地址时可弹层播放
  - 无视频地址时优雅降级到封面或提示
  - 若上游返回的站外视频/封面地址失效或拒绝访问，前端必须自动降级并明确提示，不能继续展示看似可播放但实际空白的播放器
- 阈值告警规则维护页
- 预警中心保留模板式建规则入口
- 历史快照保留
...
- 前端事件绑定必须对条件渲染或已下线模块的 DOM 节点做空值保护，禁止因为单个缺失节点导致整页脚本中断
...
- 首页总览、弹窗副标题、列表空状态的提示语必须尽量短句，避免解释性长句占据主视觉。
- 上传页、预警页、账号与权限页的主操作按钮默认使用紧凑尺寸；只有首页主动作允许保留较高权重。
- 上传页、预警页的辅助说明只保留一条关键提示，不叠加多层解释。
- 上传任务表和预警规则表优先展示状态、进度、对象和失败摘要，不保留过多可从其他列推导出的重复统计列。
...
- 页面查询接口的桌面布局，优先保证搜索框不被挤到第二排；如需压缩，应先缩短文案、按钮和日期控件宽度。
- 计划、素材、运营排名模块默认只保留横向表格，不再新增右侧详情栏作为主交互。
- 管理员新建运营账号时，允许直接写入关键词；关键词补录是补充动作，不是前置依赖。
- 运营账号命中素材的筛选口径固定为“素材名称关键词”。
- 素材预览按钮不能因为缺少直链而直接禁用，应允许打开预览弹层并展示可用信息。
